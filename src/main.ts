...
function renderAdmin() {
    ...
    <div class="container fade-in">
      ...
      <div class="card-section">
        ...
        <div class="form-group" style="position: relative;">
          <input type="password" id="admin-password" value="${state.admin.passwordHash}" style="padding-right: 40px;">
          <button type="button" onclick="window.togglePassword()" style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; font-size: 1.2rem; padding: 0; display: flex; align-items: center; justify-content: center; height: 100%;" title="Mostrar/Ocultar Senha">
  ...
  ;(window as any).togglePassword = () => {
    const input = document.querySelector('#admin-password') as HTMLInputElement
    if (input) {
...
function renderResult() {
  ...
  const resultHTML = `
    <div class="container fade-in" id="result-page">
      ...
      ${state.evaluationMode === 'DETAILED' ? `
        <div class="legend-card" style="margin-bottom: 1.5rem;">
          <strong>Legenda Sugestão Final:</strong><br><br>
          ${getLegend()}
        </div>
      ` : ''}
  ...
  document.querySelector('#download-zip-btn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const fullStyles = `
        ...
        .legend-card { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 1.5rem; }
        .legend-item { font-size: 0.85rem; margin-bottom: 0.25rem; color: var(--text-main); }
